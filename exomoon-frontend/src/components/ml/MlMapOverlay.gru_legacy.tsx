...
export function MlMapOverlay({ onClose, containerRef, onApplyAndRun }: MlMapOverlayProps) {
  ...
  // ── Per-section model type selectors ─────────────────────────────────────────
  const [predModelType, setPredModelType] = useState<'gru' | 'lstm'>('gru');
  const [histModelType, setHistModelType] = useState<'gru' | 'lstm'>('gru');
  ...
  const fetchHistory = useCallback(async (modelType?: 'gru' | 'lstm') => {
    const mt = modelType ?? 'gru';
    try {
  ...
  const fetchTrainStatus = useCallback(async () => {
    try {
      ...
      if (data.status === 'complete' || data.status === 'error' || data.status === 'failed') {
        stopPolling();
        setHistModelType(trainModelType);   // sync history view to just-trained type
        fetchHistory(trainModelType);       // refresh Section 3 charts
      }
    } catch { /* network error — keep polling */ }
  }, [stopPolling, fetchHistory, trainModelType]);
  ...
  }, [params, simYears, predModelType, setMlPrediction]);
  ...
  useEffect(() => {
    fetchHistory(histModelType);
  }, [histModelType, fetchHistory]);
      ...
      <div className="flex-1 min-h-0 overflow-y-auto text-xs text-gray-300 divide-y divide-gray-800/60">
        ...
        {sec1Open && (
          <div className="p-3 space-y-3">
            ...
            <HpLabel label="Model type">
              <select
                value={predModelType}
                onChange={e => setPredModelType(e.target.value as 'gru' | 'lstm')}
        ...
        {sec3Open && (
          <div className="p-3 space-y-2">
            {/* Model type selector for history view */}
            <HpLabel label="Model type">
              <select
                value={histModelType}
                onChange={e => setHistModelType(e.target.value as 'gru' | 'lstm')}
                className={selectCls}
              >
                <option value="gru">GRU</option>
                <option value="lstm">LSTM</option>
